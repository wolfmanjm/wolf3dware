...
TemperatureControl::~TemperatureControl()
{
	if(read_temperature_timer_handle != nullptr){
		#ifdef FREE_RTOS
		xTimerDelete((TimerHandle_t)read_temperature_timer_handle, 10);
		#endif
		#ifdef MBED_RTOS
		RtosTimer *t= (RtosTimer*)read_temperature_timer_handle;
		t->stop();
		delete t;
		#endif
	}
...
void TemperatureControl::initialize()
{
	...
	if(read_temperature_timer_handle == nullptr) {
		...
		xTimerStart((TimerHandle_t)read_temperature_timer_handle, 10);
		#endif

		#ifdef MBED_RTOS
	 	RtosTimer *t= new RtosTimer(temperatureTimerCallback, osTimerPeriodic, (void *)this);
	 	t->start(1000/readings_per_second);
...
void TemperatureControl::temperatureTimerCallback( void const *pTc )
{
#ifdef FREE_RTOS
	TemperatureControl *tc = (TemperatureControl*)pvTimerGetTimerID( (TimerHandle_t)pTc );
	tc->readTemperatureTick();
#endif
#ifdef MBED_RTOS
	TemperatureControl *tc = (TemperatureControl*)pTc;
	tc->readTemperatureTick();
#endif
}
